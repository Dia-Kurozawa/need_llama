"""
BERT-based Ensemble Model for Novel Classification Task

This script implements a BERT-based ensemble model for classifying text samples 
as either common or unique novels. The model uses soft voting ensemble technique 
...
4. Soft voting ensemble of multiple models
...
# Different random states were used in experimentation to validate model robustness
# Values used in experiments: [111, 222, 555, 666, 777]
random_state = 111
...
df_vote_name = f'df_softvote_{loss_type}_{random_state}_2-123_epoch{EPOCH}_dummy.xlsx'
df_rank_name = f'df_softrank_{loss_type}_{random_state}_2-123_epoch{EPOCH}_dummy.xlsx'

# Column names for storing prediction results
columns = ['2^1_0', '2^1_1', '2^2_0', '2^2_1', '2^3_0', '2^3_1', 'soft_vote', 'label', 'X_test']
...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        ce_loss = 0.5 * (self.loss_fct(logits_1, labels) + self.loss_fct(logits_2, labels))
...
for times, excel_name in enumerate(augmentation_files):
    ...
    val_dataset = Dataset(X_test_tokenized, y_val)
    ...
    # Attach loss function to trainer for use in compute_loss method
    trainer.loss_fct = Loss(
        loss_type=loss_type,
    ...
    # Save raw predictions for soft voting
    df_vote[columns[2*times]] = raw_pred[:, 0]      # Probability for class 0
    df_vote[columns[2*times+1]] = raw_pred[:, 1]    # Probability for class 1
...
# Perform soft voting by averaging probabilities from all models
raw_pred_0 = df_vote['2^1_0'] + df_vote['2^2_0'] + df_vote['2^3_0']
raw_pred_1 = df_vote['2^1_1'] + df_vote['2^2_1'] + df_vote['2^3_1']
raw_pred_compare = np.array([raw_pred_0, raw_pred_1]).T

# Final prediction based on highest average probability
df_vote['soft_vote'] = np.argmax(raw_pred_compare, axis=1)

# Print ensemble results
print('Soft Voting Ensemble Results')
print(classification_report(df_vote.label, df_vote.soft_vote, target_names=['Common', 'Unique'], digits=4)) 
print(confusion_matrix(df_vote.label, df_vote.soft_vote))
